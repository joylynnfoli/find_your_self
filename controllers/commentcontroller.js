...
router.post('/add/:entryId', validateSession, (req, res)=>{
    const commentEntry = {
        user_id: req.user.id,
        videoId: req.body.comment.videoId,
...
router.put("/update/:entryId", validateSession,
function (req, res) {
    ...
    Comment.update(updateCommentEntry, query).then((comment) => res.status(200).json (comment))
    .catch((err)=> res.status(500).json({error:err}));
...
function (req, res) {
    ...
    .then(() => res.status(200).json({message:"Comment Removed" })).catch((err) => res.status(500).json({
        error:err }));
});
